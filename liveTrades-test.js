...
const headers = {
  ...
  const outputFileName = 'live-trades-test.json';  
    ...
    for(let t=0;t<traders.length;t++){    
        ...
        const response = await axios.get(url, axiosConfig);
          if (response.status === 200) {                        
            ...
            let liveTrades = response.data.result.data;
            //is trade open is 1 means its hidden from profile
            console.log("is trade open: ", response.data.result.openTradeInfoProtection);
            logger.info("is trade open: ", response.data.result.openTradeInfoProtection);
            // if(response.data.result.openTradeInfoProtection === 1){
            //   updateTrader(traders[t]['trader_id']);
            // }

            // let liveTrades= liveTradesRaw.map(s=>{
            //   s.stopLossPrice = s.stopLossPrice === '' ? 0 : s.stopLossPrice;
            //   s.takeProfitPrice = s.takeProfitPrice === '' ? 0 : s.takeProfitPrice;
            //   s.reCalcEntryPrice = s.reCalcEntryPrice === '' ? 0 : s.reCalcEntryPrice;
            ...
            allData = allData.concat(liveTrades);            
            ...
            if(totalCount > 50){
              for(let i=1;i<=(totalCount/50);i++){
                ...
                if (responseNext.status === 200) {
                  let liveTradesNext = responseNext.data.result.data;
                  ...
                  allData = allData.concat(liveTradesNext);
                  console.log(`Fetched data for trader ${traders[t]['trader_id']} from position ${t} with page ${pageNo}`);
                  logger.info(`Fetched data for trader ${traders[t]['trader_id']} from position ${t} with page ${pageNo}`);
                }                
              }
...
const callApi = async (url) => {
  try {    
    ...
    if (response.status === 200) {
      ...
      console.error(`Received a ${response.status} response for URL: ${url}`);
      logger.error(`Received a ${response.status} response for URL: ${url}`);
      return [];
    }
  } catch (error) {
    console.error(`An error occurred for URL ${url}:`, error.message);
    logger.error(`An error occurred for URL ${url}: ${error.message}`);
    return [];
  }
};

const fetchAllData = async () => {
  const traders = await fetchTradersFromDatabase();

  async.parallelLimit(
    traders.map((trader) => (callback) => {
      let pageNo = 1;
      const allData = [];
      async.whilst(
        () => true,
        (next) => {
          let url = `${baseUrl}?page=${pageNo}&pageSize=50&timeStamp=${timeStamp}&leaderMark=${trader['trader_id']}`;
          callApi(url)
            .then((response) => {
              let totalCount = response.totalCount;
              let liveTradesRaw = response.data;
              let liveTrades = liveTradesRaw.map(s=>{                
                s.stopLossPrice = s.stopLossPrice === '' ? 0 : s.stopLossPrice;
                s.takeProfitPrice = s.takeProfitPrice === '' ? 0 : s.takeProfitPrice;                
                s.reCalcEntryPrice = s.reCalcEntryPrice === '' ? 0 : s.reCalcEntryPrice;                
    ...
    (err, results) => {
      if (err) {
        ...
        logger.error(`An error occurred while fetching data: ${err}`);
      } else {
        const allData = [].concat(...results); // Concatenate all data arrays
...
let traders = [];
const readCsv = async () => {
  // traders = JSON.parse(fs.readFileSync('./master-traders-test.json', 'utf8'));
  // console.log(traders);
  try{
